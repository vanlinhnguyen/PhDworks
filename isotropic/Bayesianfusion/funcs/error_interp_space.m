...
function [err_mean,err_max] = error_interp_space(space_spacing, err)
...
% mean
err_mean = err(x_ref_all,IsY_all);
...
if mod(space_spacing,2) == 0 % don't need to check time_spacing, always even in our tests
    ...
    x_interp_mid = IsY_all(:,id_firstmid:space_spacing:end,...
        id_firstmid:space_spacing:end,:);        
else
    ...
    x_interp_mid = [IsY_all(:,id_firstmid:space_spacing:end,id_firstmid:space_spacing:end,:);...
        IsY_all(:,id_firstmid+1:space_spacing:end,id_firstmid:space_spacing:end,:);...
        IsY_all(:,id_firstmid:space_spacing:end,id_firstmid+1:space_spacing:end,:);...
        IsY_all(:,id_firstmid+1:space_spacing:end,id_firstmid+1:space_spacing:end,:)];
end
