%SR_PATCHESHRLR_PATCHESLRINTERP_PATCHSIZE06_MIDPLANES  using SR1 to
...
function filename_SR = SR_patchesHR_patchesLR_patchsize06_midplanes(filename_dict, params_train, params_rec)
fprintf('\n *************** SR - COUPLE DICTS HR-LR  **************** \n');
%% INITIAL PARAMS
space_spacing=4;
time_spacing=6;
Nt = 37;
...
sizes={Nl+size_l,Nl+size_l,Nh+size_h,Nh+size_h};

%% Loop all blocks
midplane_ids = 4:time_spacing:Nh;

filename_SR=strcat('/data/PhDworks/isotropic/dictionarylearning/downsampling/SR_HR_LR_K',...
...
for t=1:Nt
    ...
    for i=1:numel(midplane_ids)
        ...
        X_LR_enlarged = nc2{'X_LR_enlarged'}(t,:,:,midplane_ids(i));
        X_LR_interp = nc2{'X_LRinterp_enlarged'}(t,bottom+1:bottom+Nh,left+1:left+Nh,midplane_ids(i));

        % SR
        start=tic();

        X_HR_rec_enlarged = superresol(X_LR_enlarged, D_HR, D_LR, params_rec,'SR1', grids, sizes);
