      ...
      case '.'
        switch theSubs
            ...
            case 'close'                                  
                    close(mDatasetObj);     
            case 'itsDimsize'                                  
                    result = mDatasetObj.itsDimsize;    
            case 'isUnlimited'                                  
                    result = mDatasetObj.isUnlimited;                        
            otherwise
      ...
      case '{}'
       ...
       case '()'
         switch class(theSubs)
            case 'char'
                mydim = theSubs;  
                ncInfo = getInfo(mDatasetObj);
                [isLength isUnlimited] = The_dimensions(ncInfo,mydim);
                result = struct('isUnlimited',isUnlimited,'itsDimsize',isLength);
                structSize = length(s);
                if (structSize == 1)
                    if ~isempty(result) 
                        result = subsref(result, s); %make result/subsref call
                    end
                end
                
            otherwise
                error(['??? Reference to non-existent dimension ' theSubs '.']);
         end         
