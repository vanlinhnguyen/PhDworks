...
hold on
pcolor(gridz,gridy,NRMSE_interp_spatial);shading interp;
...
[C,hfigc] = contour(gridz, gridy, NRMSE_interp_spatial,0.3:0.05:0.5);
...
hold on
pcolor(gridz,gridy,NRMSE_interp_temporal);shading interp;
ylim([0,1]);
set(gca, 'YTick', []);
...
hold on
pcolor(gridz,gridy,NRMSE_RR);shading interp;
ylabel('$\alpha/\Delta y$', 'interpreter', 'latex');
ylim([0,1]);
set(gca, 'YTick', 0:0.5:1);
set (gca, 'YTickLabel', {'0.0', '0.5','1.0'})
...
[C,hfigc] = contour(gridz,gridy,NRMSE_fusion_LG,0.3:0.05:0.5);
clabel(C,hfigc,'FontSize',fsize);
set(hfigc,'LineWidth',0.25,'Color', [0.5 0.5 0.5]);
hold off;box on;
...
hold on
pcolor(gridz,gridy,NRMSE_KRR);shading interp;
...
[C,hfigc] = contour(gridz,gridy,NRMSE_fusion_BF,0.3:0.05:0.5);
clabel(C,hfigc,'FontSize',fsize);
set(hfigc,'LineWidth',0.25,'Color', [0.5 0.5 0.5]);
hold off;box on;
...
hold on
pcolor(gridz,gridy,NRMSE_NLM_nongreedy);shading interp;
...
hold on
pcolor(gridz,gridy,NRMSE_fusion_BF);shading interp;
xlabel('$\beta/\Delta z$', 'interpreter', 'latex');
ylim([0,1]);
set(gca, 'YTick', []);
...
export_fig('./figures/compare_boxin4HWs_sspacing6_tspacing8', '-a1','-q101','-eps');
close();
