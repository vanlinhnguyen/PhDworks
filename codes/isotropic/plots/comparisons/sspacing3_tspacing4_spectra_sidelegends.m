...
%% Plot spectra
load sspacing3_tspacing4_spectra.mat
fsize=20;
fname='CMU Serif';
cc=[96/255 96/255 96/255];

h=figure;
set(h, 'Position', [50 50 1400 800]);
set(h,'color','w')
...
% 
ax(1)=subplot(2,2,1,'position',[0.14 0.55 0.3 0.4]); % top left
...
% =======    
ax(3)=axes('position',[0.475 0.55 0.3 0.4]); % top right
% -5/3 line
xline=[4,20];
yline=5*[exp((-5/3)*log(4)),exp((-5/3)*log(20))];
...
% =======    
ax(2)=axes('position',[0.14 0.12 0.3 0.4]); % bottom left
...
ax(4)=axes('position',[0.475 0.12 0.3 0.4]);
...
hL = legend([h11,h12,h13,h14,h15,h16], leg,'Interpreter','Latex');
legend boxoff
newPosition = [0.82 0.45 0.12 0.12];
set(hL,'Position', newPosition,'box', 'off');

export_fig('./figures/spectra_sspacing3_tspacing4_spectra2d_sidelegends','-a1','-q101','-eps','-painters');
close()     