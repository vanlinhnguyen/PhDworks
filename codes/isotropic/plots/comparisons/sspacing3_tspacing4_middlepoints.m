...
HTLS_ids = [2:space_spacing:96 3:space_spacing:96];
%% Load data
...
x_ref_all=nc1{'velocity_x'}(:,:,:,:); x_ref_all=x_ref_all(:,HTLS_ids,HTLS_ids,1:1:LTHS_idt(end));
x_interp_spatial_all=nc2{'Uinterp'}(:,:,:,:);x_interp_spatial_all=x_interp_spatial_all(:,HTLS_ids,HTLS_ids,1:1:LTHS_idt(end));
x_interp_temporal_all=nc3{'Uinterp'}(:,:,:,:);x_interp_temporal_all=x_interp_temporal_all(:,HTLS_ids,HTLS_ids,1:1:LTHS_idt(end));
x_fusion_BF_all=nc4{'Zhat_all'}(:,:,:,:);x_fusion_BF_all=x_fusion_BF_all(:,HTLS_ids,HTLS_ids,1:1:LTHS_idt(end));
x_fusion_LG_all=nc5{'Zhat_all'}(:,:,:,:);x_fusion_LG_all=x_fusion_LG_all(:,HTLS_ids,HTLS_ids,1:1:LTHS_idt(end));

x_NLM_greedy_all=permute(nc6{'x_HR_NLM_smallscales_all'}(:,:,:,:),[1,3,4,2]);
x_NLM_greedy_all=x_NLM_greedy_all(:,HTLS_ids,HTLS_ids,1:1:LTHS_idt(end))+x_interp_spatial_all;

x_NLM_nongreedy_all=permute(nc7{'x_rec_all'}(:,:,:,:),[1,3,4,2]);
x_NLM_nongreedy_all=x_NLM_nongreedy_all(:,HTLS_ids,HTLS_ids,1:1:LTHS_idt(end))+x_interp_spatial_all;

x_RR_all=nc8{'Urec'}(:,:,:,:);x_RR_all=x_RR_all(:,HTLS_ids,HTLS_ids,1:1:LTHS_idt(end));
x_KRR_all=nc9{'Urec'}(:,:,:,:);x_KRR_all=x_KRR_all(:,HTLS_ids,HTLS_ids,1:1:LTHS_idt(end));
...
set (gca, 'YTickLabel', {'0.00','0.05','0.10','0.15','0.20','0.25'},'FontSize',fsize)
...
print
export_fig('./figures/compare_NRMSE_time_middlepoints_sspacing3_tspacing4','-eps');
close();
