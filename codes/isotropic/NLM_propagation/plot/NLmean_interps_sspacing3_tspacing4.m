...
%%
% save NLmean_interps_sspacing3_tspacing4.mat ...
%      NRMSE_interp_space_mean NRMSE_interp_space_std ...
%      NRMSE_interp_time_mean NRMSE_interp_time_std ...
%      NRMSE_NLmeans1_mean NRMSE_NLmeans1_std ...
%      NRMSE_NLmeans2_mean NRMSE_NLmeans2_std ...
%      E_ref1 E_interp_space1 E_err_interp_space1 ...
%      E_interp_time1 E_err_interp_time1 ...
%      E_NLmean11 E_err_NLmean11 E_NLmean12 E_err_NLmean12 ...
%      E_ref2 E_interp_space2 E_err_interp_space2 ...
%      E_interp_time2 E_err_interp_time2 ...
%      E_NLmean21 E_err_NLmean21 E_NLmean22 E_err_NLmean22 ...

load NLmean_interps_sspacing3_tspacing4.mat
%% Plot NRMSE
fsize=28;
fname='CMU Serif';
...
leg={'$\mathbf{I}_s\mathbf{y}$','$\mathbf{I}_t\mathbf{x}$','Greedy propagation','Non-greedy propagation'};
...
export_fig('./figures/NLmean_interps_sspacing3_tspacing4_NRMSE', '-a1','-q101','-eps','-painters'); % leave '-painters' can cause quality problems
close()
...
leg={'Reference','Greedy propagation','$\mathbf{I}_s \mathbf{y}$','Non-greedy propagation','$\mathbf{I}_t\mathbf{x}$'};
legendflex([h11,h14,h12,h15,h13],leg, 'ref', gcf, ...
                       'anchor', {'n','n'}, ...
                       'buffer',[0 0], ...
                       'nrow',2, ...
                       'fontsize',fsize,...
                       'Interpreter','Latex',...
                       'box','off');
% export_fig('./figures/NLmean_interps_sspacing3_tspacing4_spectra2d','-a1','-q101','-eps','-painters');
% close()     


%% Plot re-scaled spectra
fsize=20;
fname='CMU Serif';
...
h11=loglog(3:k_max,(3:k_max)'.^(5/3).*E_ref1(3:k_max),'k-','LineWidth',2); 
hold on;
h12=loglog(3:k_max,(3:k_max)'.^(5/3).*E_interp_space1(3:k_max),'g-','LineWidth',2); 
h13=loglog(3:k_max,(3:k_max)'.^(5/3).*E_interp_time1(3:k_max),'m-','LineWidth',2);
h14=loglog(3:k_max,(3:k_max)'.^(5/3).*E_NLmean11(3:k_max),'r-','LineWidth',2); 
h15=loglog(3:k_max,(3:k_max)'.^(5/3).*E_NLmean12(3:k_max),'b-','LineWidth',2); 
plot([k_cutoff,k_cutoff] ,[1e-8,1e0],'k--','LineWidth',2)
hold off
xlim([2,60]);
ylim([1e-2,1e1]);
text(6,1.5*1e-2,'$t/\delta t = 1$','Interpreter','Latex','HorizontalAlignment','right','FontSize',fsize+5)
...
ylabel('$k^{5/3}E(k)$','interpreter','latex');
...
h21=loglog(3:k_max,(3:k_max)'.^(5/3).*E_ref2(3:k_max),'k-','LineWidth',2); 
hold on;
h22=loglog(3:k_max,(3:k_max)'.^(5/3).*E_interp_space2(3:k_max),'g-','LineWidth',2); 
h23=loglog(3:k_max,(3:k_max)'.^(5/3).*E_interp_time2(3:k_max),'m-','LineWidth',2);
h24=loglog(3:k_max,(3:k_max)'.^(5/3).*E_NLmean21(3:k_max),'r-','LineWidth',2); 
h25=loglog(3:k_max,(3:k_max)'.^(5/3).*E_NLmean22(3:k_max),'b-','LineWidth',2); 
plot([k_cutoff,k_cutoff] ,[1e-8,1e0],'k--','LineWidth',2)
hold off
xlim([2,60]);
ylim([1e-2,1e1]);
text(6,1.5*1e-2,'$t/\delta t = 2$','Interpreter','Latex','HorizontalAlignment','right','FontSize',fsize+5)
...
legendflex([h11,h14,h12,h15,h13],leg, 'ref', gcf, ...
                       ...
                       'Interpreter','Latex',...
                       'box','off');
export_fig('./figures/NLmean_interps_sspacing3_tspacing4_spectra2d_rescaled','-a1','-q101','-eps','-painters');
close()     
...
% ylim([3*1e-6,1]);
% 
% leg=legend([h1 h2 h3 h4 h5],{'Reference','$\mathbf{I}_s \mathbf{y}$','$\mathbf{I}_t\mathbf{x}$','Greedy propagation','Non-greedy propagation'},'location','southwest');
...
% xlabel('k'); ylabel('E(k)');
% 
% leg=legend([h1 h2 h3 h4],{'$\mathbf{I}_s \mathbf{y}$','$\mathbf{I}_t\mathbf{x}$','Greedy propagation','Non-greedy propagation'},'location','northwest');
