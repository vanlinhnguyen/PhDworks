%FUSION_LINEARGAUSS fusion using linear Gaussian model
%
%IN:
...
function fusion_linearGauss(space_spacing, time_spacing)
fprintf('\n ========================================================= \n'); 
fprintf('\n LINEAR GAUSSIAN: (SSPACING,TSPACING) = (%.1d,%.1d) \n', space_spacing,time_spacing); 
fprintf('\n ========================================================= \n'); 

%% Define file locations (to load or to save)

filename_ref='/data/ISOTROPIC/data/data_downsampled4.nc';
filename_fusion=strcat('/data/PhDworks/isotropic/Bayesianfusion/FusedData_LG_tspacing',num2str(time_spacing,'%.2d'),'sspacing_',num2str(space_spacing,'%.2d'),'.nc');
...
var_z = nc1{'velocity_x'}(:,:,:,LTHS_idt).^2;
var_Isy = IsY_all(:,:,:,LTHS_idt).^2;
var_hs=hs.^2; clearvars hs;
close(nc1);

S_hs=squeeze(sum(var_hs,1));
S_hs=squeeze(sum(S_hs,3));
S_hs=S_hs./(Nt*numel(LTHS_idt));

% average
S_ave_hs=zeros(Ny,Nz);
for i=1:space_spacing
    for j=1:space_spacing
        S_ave_hs(i:space_spacing:Ny,j:space_spacing:Nz) = mean(mean(S_hs(i:space_spacing:Ny,j:space_spacing:Nz)));
    end  
end

S_z = ones(Ny,Nz)*mean(mean(mean(mean(var_z))));
S_Isy = ones(Ny,Nz)*mean(mean(mean(mean(var_Isy))));

% ==== Compute Cnt
T_ave_ht=zeros(time_spacing,1);
T_ave_Itx=zeros(time_spacing,1);

nc1 = netcdf(filename_ref,'r');
for i=1:time_spacing
    ht = nc1{'velocity_x'}(:,HTLS_idz,HTLS_idy,i:time_spacing:Nx)-ItX_all(:,HTLS_idz,HTLS_idy,i:time_spacing:Nx);
    T_ave_ht(i) = mean(ht(:).^2);
    T_ave_Itx(i) = mean(mean(mean(mean(ItX_all(:,HTLS_idz,HTLS_idy,i:time_spacing:Nx).^2))));
end
close(nc1);
...
for t=1:Nt
    ...
    for i=1:Nx
        ...
        if(pos_t==1)
            ...
            term1=(S_ave_hs*T_ave_ht(pos_t))/T_ave_Itx(pos_t);
            term2=(S_ave_hs*T_ave_ht(pos_t))./S_Isy;
            term3=(S_ave_hs*T_ave_ht(pos_t))./S_z;
            den=S_ave_hs+T_ave_ht(pos_t)+term3;
            Wt = (S_ave_hs+term1)./den;
            Ws = (T_ave_ht(pos_t)+term2)./den;
        end
        % fusion
