...
U_MAP_all = nc4{'Zhat_all'}(1:Nt,point_id_y_MAP,point_id_z_MAP);
close(nc4); 

nc5 = netcdf('/data/DNSDATA/Fusion/BayesFusion_112014/LinearRegression/RESULTS_PRED_RR_U_sspacing_10_tspacing_10.nc','r');
...
set(gcf, 'Position', [200 200 1800 500]);
set(gcf, 'Color', 'w');

ax(1)=subplot(3,1,1, 'Position', [0.075, 0.175, 0.4, 0.75]);
hold on
plot(1:Nt,U_org,'k','LineWidth',2);
plot(1:Nt,U_MAP,'r','LineWidth',1.5);
plot(1:time_spacing:Nt,U_org(1:time_spacing:Nt),'ko','LineWidth',1,'MarkerEdgeColor','k','MarkerFaceColor','r','MarkerSize',5);
...
leg =legend ({'Reference','Fusion model','LTHS positions'},'interpreter', 'latex','location','southeast'); 

set(leg,'FontSize',fsize-2);
legend boxoff
box on

ax(2)=axes('position',[0.56, 0.175, 0.20, 0.75]);
hold on
h1=plot(ts,U_org(ts),'k','LineWidth',2);
h2=plot(ts,U_MAP(ts),'r','LineWidth',3);
h3=plot(ts,U_interp_spatial(ts),'m','LineWidth',2);
h4=plot(ts,U_interp_temporal(ts),'c','LineWidth',2);
h6=plot(ts(1:time_spacing:end),U_org(ts(1:time_spacing:end)),'ko','LineWidth',1,'MarkerEdgeColor','k','MarkerFaceColor','r','MarkerSize',5);
hold off
xlabel('$t/\delta t$','interpreter', 'latex');
...
leg =legend ([h3 h4],{'$\mathbf{I}_s \mathbf{y}$','$\mathbf{I}_t \mathbf{x}$'},'interpreter', 'latex','location','south'); 
...
ax(3)=axes('position',[0.785, 0.175, 0.20, 0.75]);
hold on
h1=plot(ts,U_org(ts),'k','LineWidth',2);
h2=plot(ts,U_MAP(ts),'r','LineWidth',3);
h3=plot(ts,U_RR(ts),'b','LineWidth',2);
h6=plot(ts(1:time_spacing:end),U_org(ts(1:time_spacing:end)),'ko','LineWidth',1,'MarkerEdgeColor','k','MarkerFaceColor','r','MarkerSize',5);
hold off
xlabel('$t/\delta t$','interpreter', 'latex');
% ylabel('$z_s$','interpreter', 'latex');
...
leg =legend (h3,{'Penalized LSE'},'interpreter', 'latex','location','south'); 
