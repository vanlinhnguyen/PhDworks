...
outer_id_z_fulldomain=24:264; % HWs (in z) are at 4:5:284 (but remove border effect by LSQspline)
...
nc=netcdf('/data/DNSDATA/Fusion/BayesFusion_112014/fused/variousspacings/MAP_improper_diagCn_timespacing_10_spacespacing_10_largescale.nc','r');
U_org=nc{'U_org_filtered'}(ttd,:,:); 
U_interp_spatial=nc{'U_interp_space_filtered'}(ttd,:,:);
U_interp_temporal=nc{'U_interp_time_filtered'}(ttd,:,:);
U_MAP=nc{'U_MAP_filtered'}(ttd,:,:);
close(nc);
...
filename=strcat('./Figures/improper_outer_largescales_sspacing_10_tspacing_10_subplots_t',num2str(ttd,'%.3d'));
