...
% -5/3 line
xline=[1,5];
yline=0.3*10^-1*[exp((-5/3)*log(1)),exp((-5/3)*log(5))];
...
h3=loglog(f(2:floor(window/2)),Pxx_RR(2:end-1),'b-','LineWidth',3);
% h4=loglog(f(2:floor(window/2)),Pxx_interp_space(2:end-1),'g-','LineWidth',2);
h5=loglog(f(2:floor(window/2)),Pxx_interp_time(2:end-1),'c-','LineWidth',3);
...
% xlabel('$f$', 'interpreter', 'latex'); ylabel('$E_{11}(f)$', 'interpreter', 'latex');
xlabel('Frequency'); ylabel('Energy');

leg=legend([h1 h2 h3 h5],{'Reference','Fusion model','Penalized LSE','$\mathbf{I}_t \mathbf{x}$'},'interpreter', 'latex','location','northeast');
...
h3=loglog(f(3:12),Pxx_RR(3:12),'b-','LineWidth',3.5);
% h4=loglog(f(3:12),Pxx_interp_space(3:12),'g-','LineWidth',2);
h5=loglog(f(3:12),Pxx_interp_time(3:12),'c-','LineWidth',3.5);
