...
x_MAP_all=nc5{'Zhat_all'}(i_HWs_2save,point_id_y_fulldomain,point_id_z_fulldomain);

x_nonoise=[]; x_interp_spatial=[]; x_interp_temporal=[];
x_RR=[]; x_MAP=[];
for k=point_id_z_fulldomain(1:space_spacing:end-space_spacing)
    point_id_z_onebox=(k:k+space_spacing)-3;
...
save('improper_diagCn_sspacing_10_tspacing_10_boxin4HWs_outer.mat','RMSE_interp_spatial','RMSE_interp_temporal','RMSE_RR','RMSE_MAP')


%% Plot
load improper_diagCn_sspacing_10_tspacing_10_boxin4HWs_outer.mat
...
h=figure;
set(h, 'Position', [100 100 850 650]);
set(h,'color','w')
...
export_fig('./figures/NRMSE_boxin4HWs_channel','-eps');
close();
