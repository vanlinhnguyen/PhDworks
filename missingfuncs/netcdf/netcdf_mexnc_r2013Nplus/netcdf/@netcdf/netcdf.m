...
if isempty(thePermission), thePermission = 'r'; end
...
case {'r','read'}
    result = mDataset(theFilename);
otherwise
   help netcdf
   warning([' ## No such permission: ' thePermission])
end

if ~strcmp(thePermission,'r') & ~strcmp(thePermission,'read') 
    if ncid(result) >= 0
...
perms = {'clobber', 'noclobber', 'write', 'nowrite', 'readonly', 'define','r','read'};
