function knots = create_knots_optimal(tau,k,maxiter)
...
for iter = 1:maxiter
   ais = [signs*find_spcol(tauext,k+1,xi,'sp') -1/2];     % a_1,...,a_n  from (18)
   ...
   dx = signs.* ...
      ((-cumais(1:nmk).*(tau(k+[1:nmk])-tau([1:nmk]))/k)/find_spcol(tau,k,xi,'sp'));
   ...
   if checks>0
      error('SPLINES:OPTKNT:noconv', ...
      'Damped Newton iteration failed to converge.')
   end
   if maxdx<tol, knots = create_knots_augment([tau(1) xi tau(end)],k); return, end
end
error('SPLINES:OPTKNT:maxiter',...
'Newton iteration failed to converge in %d steps.',maxiter);
